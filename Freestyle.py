...
def open_camera(camera_view_placeholder):
    ...
    tools_path="freestyle_newest.png"
    tools = cv2.imread("freestyle_newest.png")
    if tools is None:
    ...
    while True:
        ...
        # Displaying tools
        frm[:max_y, margin_left:max_x] = cv2.addWeighted(tools, 0.7, frm[:max_y, margin_left:max_x], 0.5, 0)
...
def main():
    ...
    if go_back_button:
        ...
        return
    st.sidebar.markdown('<div style="font-size:48px;"> Buttons </div>', unsafe_allow_html=True)
